...
export default function AdminScoreRow({ score, userId }: { score: any, userId: string }) {
  const [isEditing, setIsEditing] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isDeleting, setIsDeleting] = useState(false)
  // Format date to YYYY-MM-DD for the HTML5 date input
  const formattedDate = score.date ? new Date(score.date).toISOString().split('T')[0] : ''
  const [editData, setEditData] = useState({ date: formattedDate, points: score.score })
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    const res = await adminEditScore(formData)
  ...
  const handleDelete = async () => {
    ...
    formData.append('userId', userId)
    await adminDeleteScore(formData)
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5 hover:border-white/10 transition-colors">
      <div>
        <div className="font-medium text-emerald-400">{score.score} Points</div>
        <div className="text-xs text-zinc-500">{new Date(score.date).toLocaleDateString()}</div>
      </div>
